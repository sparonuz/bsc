...
!$AGRIF_END_DO_NOT_TREAT
   ...
   INTEGER         , PUBLIC ::   nn_comm_mod = 1         !: frequency of communication calls (1 = every time step)
   LOGICAL         , PUBLIC ::   ln_comm_only = .FALSE.  !: replace step routine by realistic communications only
  
   INTEGER         , PUBLIC ::   ncom_stp = 0                 !: copy of time step # istp
   INTEGER         , PUBLIC ::   icomm_sequence(500,2) = 0    !: size of communicated arrays (halos)
   INTEGER         , PUBLIC ::   n_sequence = 0               !: # of communicated arrays
   ...
   LOGICAL ::           l_print_comm_report = .TRUE. 

   !!----------------------------------------------------------------------
   !! NEMO/OCE 4.0 , NEMO Consortium (2018)
   !! $Id: lib_mpp.F90 9667 2018-05-28 15:47:05Z smasson $
...
CONTAINS
   ...
   FUNCTION mynode( ldtxt, ldname, kumnam_ref, kumnam_cfg, kumond, kstop, localComm )
      ...
      !
      NAMELIST/nammpp/ cn_mpi_send, nn_buffer, jpni, jpnj, jpnij, ln_nnogather, nn_comm_mod, ln_comm_only
      !!----------------------------------------------------------------------
      !
      ii = 1
...
!!#  undef DIM_4d
   ...
   SUBROUTINE tic_tac (l_tic)

    LOGICAL, INTENT(IN) :: l_tic
    LOGICAL, SAVE :: l_write_wt = .TRUE.
    REAL(wp),SAVE  ::   tic_wt=0._wp, tac_wt=0._wp, tic_ct=0._wp, tac_ct=0._wp

    ! start count
    IF ( l_tic ) THEN
       IF ( ncom_stp > nit000 ) tic_wt = MPI_Wtime()
       IF ( tic_ct > 0.0_wp ) tac_ct = tac_ct + MPI_Wtime() - tic_ct
    ! stop count and sum up
    ELSE
       IF ( ncom_stp > nit000 ) tac_wt = tac_wt + MPI_Wtime() - tic_wt
       IF ( ( ncom_stp == ( nitend - nn_comm_mod ) ) .AND. l_write_wt ) then
          WRITE(6,'(A20,F11.6,A15,I8)') 'Computing time : ',tac_ct,' on MPI rank : ', narea
          WRITE(6,'(A20,F11.6,A15,I8)') 'Waiting time : ',tac_wt,' on MPI rank : ', narea
          l_write_wt = .FALSE.
       END IF
       IF ( ncom_stp > nit000 ) tic_ct = MPI_Wtime()
    ENDIF
...
CONTAINS
   ...
   SUBROUTINE simulated_lbc_lnk ()
      WRITE(*,*) 'simulated_lbc_lnk: You should not have seen this print! error?'
   END SUBROUTINE simulated_lbc_lnk ()
#endif

...
!$AGRIF_END_DO_NOT_TREAT
   ...
   !! $Id: lib_mpp.F90 10815 2019-03-29 17:15:56Z smasson $
...
CONTAINS
   ...
   FUNCTION mynode( ldtxt, ldname, kumnam_ref, kumnam_cfg, kumond, kstop, localComm )
      ...
      !
      NAMELIST/nammpp/ cn_mpi_send, nn_buffer, jpni, jpnj, ln_nnogather, nn_comm_mod
      !!----------------------------------------------------------------------
      !
      ii = 1
...
#  undef OPERATION_MAXLOC
   ...
   SUBROUTINE tic_tac (ld_tic, ld_global)
    ...
    IF( ncom_stp == ( nitend - nn_comm_mod ) ) RETURN
    ii = 1
